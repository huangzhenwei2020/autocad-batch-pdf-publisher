...
namespace BatchPdfPublisher.Views
{
    internal sealed class LineVisionForm : DpiAwareForm
    {
        ...
        private void Build()
        {
            ...
            split.Panel1.Controls.Add(_preview); split.Panel2.Controls.Add(_objects); root.Controls.Add(split, 0, 1);
        ...
        private async Task AnalyzeAsync()
        {
            ...
            try
            {
                ...
                var result = await Task.Run(() => LineVisionProcessor.Analyze(path, region, settings, cancellation.Token, (percent, message) => ((IProgress<Tuple<int, string>>)progress).Report(Tuple.Create(percent, message))));
                ...
                _preview.SetResult(result); _view.SelectedIndex = 0; SyncObjects(); UpdateStatus();
            }
        ...
        private async Task InsertAsync()
        {
            ...
            Hide(); Exception failure = null; var count = 0;
            try { await CadCommandContext.ExecuteAsync(() => count = LineVisionCadWriter.PromptAndInsert(_document, _result, scale)); }
            ...
            else if (count > 0) MessageBox.Show(this, "已插入 " + count + " 根可编辑 CAD 直线。", Text, MessageBoxButtons.OK, MessageBoxIcon.Information);
        }
        ...
        private void UpdateStatus()
        {
            ...
            _status.Text = "识别 " + _result.Segments.Count + " 根，已选择 " + enabled + " 根。OCR Worker 未安装，本次仅转换线稿。";
        }
        ...
        private void SaveSettings()
        {
            try
            {
                ...
                File.WriteAllLines(SettingsPath, new[] { "threshold=" + _threshold.Text, "minimum=" + _minimum.Text, "closeGap=" + _closeGap.Text, "collinear=" + _collinear.Text, "mergeGap=" + _mergeGap.Text, "scale=" + _scale.Text, "diagonals=" + (_diagonals.Checked ? "1" : "0") });
            }
            catch { }
        }
