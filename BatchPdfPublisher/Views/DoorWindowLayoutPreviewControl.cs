        ...
        private void PaintPreview(Graphics graphics)
        {
            ...
            using (var brush = new SolidBrush(Color.FromArgb(25, 36, 48)))
            {
                for (var page = 0; page < _plan.PageCount; page++)
                {
                    ...
                    // 门窗表占位：第一页内容区底部（说明文字之上，与 ComputeLayout 预留的 ScheduleHeight 对应）。
                    if (page == 0 && _options.IncludeSchedule && _plan.ScheduleHeight > 0d)
                    {
                        var scheduleH = Safe((float)_plan.ScheduleHeight * _zoom, 0f);
                        var notesH = Safe((float)_plan.ScheduleNotesHeight * _zoom, 0f);
                        if (scheduleH > 0f)
                        {
                            var scheduleTop = rect.Y + (float)(_plan.PageHeight - (_plan.ContentBottom + _plan.ScheduleNotesHeight + _plan.ScheduleHeight)) * _zoom;
                            using (var schedulePen = new Pen(Color.FromArgb(90, 140, 90), 1.4f))
                            {
                                DrawRectSafe(graphics, schedulePen, contentLeft, scheduleTop, contentW, scheduleH, "schedule");
                                graphics.FillRectangle(new SolidBrush(Color.FromArgb(28, 230, 245, 230)), new RectangleF(contentLeft, scheduleTop, contentW, scheduleH));
                                DrawCentered(graphics, "门窗表（第一页底部，与门窗立面一起插入）", new RectangleF(contentLeft, scheduleTop, contentW, scheduleH), Color.FromArgb(40, 90, 40), 9F);
                            }
                        }
                        // 设计说明占位：第一页内容区最底部（门窗表之下，与 ComputeLayout 预留的 ScheduleNotesHeight 对应）。
                        if (_options.IncludeScheduleNotes && notesH > 0f)
                        {
                            var notesTop = rect.Y + (float)(_plan.PageHeight - (_plan.ContentBottom + _plan.ScheduleNotesHeight)) * _zoom;
                            using (var notesPen = new Pen(Color.FromArgb(130, 120, 60), 1.2f))
                            {
                                DrawRectSafe(graphics, notesPen, contentLeft, notesTop, contentW, notesH, "notes");
                                graphics.FillRectangle(new SolidBrush(Color.FromArgb(26, 250, 248, 220)), new RectangleF(contentLeft, notesTop, contentW, notesH));
                                DrawCentered(graphics, "门窗/幕墙设计说明（18 条，第一页最底部）", new RectangleF(contentLeft, notesTop, contentW, notesH), Color.FromArgb(110, 90, 30), 8.5F);
                            }
                        }
                    }
