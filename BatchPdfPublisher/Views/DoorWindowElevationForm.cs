...
namespace BatchPdfPublisher.Views
{
    internal sealed class DoorWindowElevationForm : Form
    {
        ...
        private static readonly string[] DoorWindowTypes = { "普通窗", "高窗", "带形窗", "转角窗", "拱形窗", "凸窗", "百叶窗", "甲级防火窗", "乙级防火窗", "丙级防火窗", "防火窗（等级待确认）", "普通门", "甲级防火门", "乙级防火门", "丙级防火门", "防火门（等级待确认）", "人防门", "百叶门", "门联窗", "洞口", "待确认" };
        ...
        private readonly ComboBox _frameChoice = new ComboBox { DropDownStyle = ComboBoxStyle.DropDownList, Width = 210, Height = 28, Margin = new Padding(4, 3, 4, 0), Enabled = false };
        private readonly CheckBox _insertSchedule = new CheckBox { Text = "同时插入门窗表", AutoSize = true, Margin = new Padding(4, 7, 4, 0), Enabled = false };
        private readonly CheckBox _insertNotes = new CheckBox { Text = "门窗设计说明", AutoSize = true, Margin = new Padding(4, 7, 4, 0), Enabled = false };
        ...
        private void Build()
        {
            ...
            batch.Controls.Add(_insertFrame); LoadFrameChoices(); batch.Controls.Add(_frameChoice);
            batch.Controls.Add(_insertSchedule); batch.Controls.Add(_insertNotes); batch.Controls.Add(_useTianzhengTitle);
            try
            {
                var saved = DoorWindowLayoutPreviewForm.LoadSavedMargins();
                _insertSchedule.Checked = saved.IncludeSchedule;
                _insertNotes.Checked = saved.IncludeScheduleNotes;
                _useTianzhengTitle.Checked = saved.UseTianzhengTitle;
            }
            catch { }
            _insertFrame.CheckedChanged += (s, e) => { _frameChoice.Enabled = _insertFrame.Checked; _insertSchedule.Enabled = _insertFrame.Checked; _insertNotes.Enabled = _insertFrame.Checked && _insertSchedule.Checked; };
            _insertSchedule.CheckedChanged += (s, e) => { _insertNotes.Enabled = _insertFrame.Checked && _insertSchedule.Checked; };
...
#endif
        {
            ...
            var scale = ParseDrawingScale(); var frame = SelectedFrame(); SavePreferences(false);
            // 勾选“插入图框排版”时先弹排版预览：可调排版范围、拖拽门窗顺序，
            // 确认后按调整后的顺序与参数插入。
            DoorWindowElevationInsertionService.DoorWindowLayoutOptions layoutOptions = null;
            if (frame != null)
            {
                using (var preview = new DoorWindowLayoutPreviewForm(ready, scale, frame, _insertSchedule.Checked, _insertNotes.Checked, _useTianzhengTitle.Checked))
                {
        ...
        private void LoadFrameChoices()
        {
            ...
            if (_frameChoice.Items.Count == 0) _frameChoice.Items.Add(new FrameChoice { Name = "（请先登记图框）" });
            _frameChoice.SelectedIndex = 0;
        }

        private FrameDefinition SelectedFrame()
        { return _insertFrame.Checked ? (_frameChoice.SelectedItem as FrameChoice)?.Frame : null; }
        ...
        private void OnGridCellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            ...
            var editableBatchProperty = property == "ElevationType" || property == "DivisionPreset" || property == "OpeningMode" || property == "HasInstallationGap" || property == "InstallationGap" || property == "HasOuterFrame" || property == "OuterFrameWidth" || property == "HasMullion" || property == "MullionWidth" || property == "DoorFrameType" || property == "Material" || property == "SillHeightDisplay" || property == "AtlasName" || property == "Remarks";
