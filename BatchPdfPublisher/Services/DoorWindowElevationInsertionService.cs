...
namespace BatchPdfPublisher.Services
{
    internal static class DoorWindowElevationInsertionService
    {
        ...
        private sealed class ElevationPlacement
        {
            ...
            public double InnerGap, OuterGap, LowerExtent;
        }
        ...
        public sealed class DoorWindowLayoutOptions
        {
            ...
            /// <summary>是否同时插入门窗表：门窗表排在第一页内容区顶部，门窗立面从其下方开始排。</summary>
            public bool IncludeSchedule = false;
            /// <summary>是否同时插入门窗设计说明：说明文字排在第一页门窗表下方。</summary>
        ...
        public static DoorWindowLayoutPlan ComputeLayout(IList<DoorWindowScheduleItem> source, int drawingScale, FrameDefinition frame, DoorWindowLayoutOptions options)
        {
            ...
            // 门窗表：占第一页内容区顶部，宽度不超过内容区（超宽按比例压缩行高保持比例）。
            var scheduleHeight = 0d;
            if (opts.IncludeSchedule)
            {
                var contentWidth = contentRight - contentLeft;
                var scheduleFactor = Math.Min(1d, contentWidth / (221d * scale));
                scheduleHeight = (items.Count + 2) * 7d * scale * scheduleFactor;
                if (scheduleHeight > contentTop - contentBottom)
                    throw new InvalidOperationException("门窗表有 " + (items.Count + 2) + " 行，占 " + Math.Ceiling(scheduleHeight / scale) + "mm 高，放不进所选 " + frame.PaperDisplay + " 图框的第一页。");
            }
            plan.ScheduleHeight = scheduleHeight;
            // 设计说明：排在第一页门窗表下方，占 contentTop - scheduleHeight 以下区域。
            var scheduleNotesHeight = 0d;
            if (opts.IncludeScheduleNotes)
            {
                scheduleNotesHeight = EstimateScheduleNotesHeight(contentRight - contentLeft, scale);
                if (scheduleHeight + scheduleNotesHeight > contentTop - contentBottom)
                    throw new InvalidOperationException("门窗表与设计说明合计占 " + Math.Ceiling((scheduleHeight + scheduleNotesHeight) / scale) + "mm 高，放不进所选 " + frame.PaperDisplay + " 图框的第一页。请加大图框或取消设计说明。");
            }
            plan.ScheduleNotesHeight = scheduleNotesHeight;
            // 顺序：先左右后上下——第一行在顶部，同一行从左到右排满后换到下一行。
            // slot.X/slot.Y 统一为立面插入原点（左下角，相对页原点，模型单位）。
            // 门窗表+设计说明放第一页底部：第一页门窗排布下限 = contentBottom + 表高 + 说明高 + 间距。
            var firstPageBottom = contentBottom + scheduleHeight + scheduleNotesHeight + Math.Max(8d, opts.ItemGap) * scale;
            ...
            Func<DoorWindowScheduleItem, int, bool, bool> place = (item, page, locked) =>
            {
                ...
                var footprintWidth = placement.OuterGap + item.Width + itemGap;
                ...
                var bottomLimit = page == 0 ? firstPageBottom : contentBottom;
                ...
                plan.Slots.Add(new DoorWindowLayoutSlot { Item = item, Page = page, X = cursor.X + placement.OuterGap, Y = cursor.Y - item.Height, FootprintWidth = footprintWidth, FootprintHeight = footprintHeight });
        ...
        /// <summary>构建门窗表 Table（未加入图形）。宽度恒为 8 列；Position 为表格左上角。</summary>
        ...
        private static ElevationPlacement CreatePlacement(DoorWindowScheduleItem item, int drawingScale)
        {
            ...
            return new ElevationPlacement { Item = item, InnerGap = innerGap, OuterGap = outerGap, LowerExtent = outerGap + 12d * drawingScale };
        }
        ...
        private static void InsertContinuous(IList<ElevationPlacement> elevations, Point3d origin, int scale, BlockTableRecord space, Transaction transaction, DraftingStandardResources resources, ObjectId dimensionStyle, Action<int, int, string> progress, bool useTianzhengTitle = true)
        {
            ...
            {
                InsertElevation(elevation, new Point3d(x, origin.Y, origin.Z), scale, space, transaction, resources, dimensionStyle, null, useTianzhengTitle);
                // 连续排列间距按标注外框算：左侧尺寸标注 OuterGap + 洞口宽 + 间距。
                x += elevation.OuterGap + elevation.Item.Width + Math.Max(16d * scale, 800d);
        ...
        private static int InsertPaged(IList<ElevationPlacement> elevations, Point3d origin, int scale, FrameDefinition frame, BlockTable blockTable, BlockTableRecord space, Transaction transaction, DraftingStandardResources resources, ObjectId dimensionStyle, Action<int, int, string> progress, DoorWindowLayoutOptions layoutOptions = null)
        {
            ...
            var completed = 0;
            var includeSchedule = layoutOptions != null && layoutOptions.IncludeSchedule && plan.ScheduleHeight > 0d;
            for (var pageIndex = 0; pageIndex < plan.PageCount; pageIndex++)
            {
                ...
                // 门窗表：只插入第一页，位于内容区底部（说明文字之上，与 ComputeLayout 预留的 ScheduleHeight 对应）。
                if (pageIndex == 0 && includeSchedule)
                {
                    var tableTop = plan.ContentBottom + plan.ScheduleNotesHeight + plan.ScheduleHeight;
                    var table = BuildScheduleTable(space.Database, elevations.Select(x => x.Item).ToList(), scale, new Point3d(pageOrigin.X + plan.ContentLeft, pageOrigin.Y + tableTop, origin.Z));
                    space.AppendEntity(table); transaction.AddNewlyCreatedDBObject(table, true); table.GenerateLayout();
                }
                // 设计说明：只插入第一页，位于内容区最底部（门窗表之下，与 ComputeLayout 预留的 ScheduleNotesHeight 对应）。
                if (pageIndex == 0 && layoutOptions != null && layoutOptions.IncludeScheduleNotes && plan.ScheduleNotesHeight > 0d)
                {
                    var notesOrigin = new Point3d(pageOrigin.X + plan.ContentLeft, pageOrigin.Y + plan.ContentBottom + plan.ScheduleNotesHeight, origin.Z);
                    var notes = new MText
                    {
                        Contents = string.Join("\\P", ScheduleNotes),
                        Location = notesOrigin,
                        Attachment = AttachmentPoint.TopLeft,
                        TextHeight = 3.5d * scale,
                        LineSpacingFactor = 1.5d,
                        LineSpacingStyle = LineSpacingStyle.AtLeast,
                        Width = Math.Max(1d, plan.ContentRight - plan.ContentLeft),
