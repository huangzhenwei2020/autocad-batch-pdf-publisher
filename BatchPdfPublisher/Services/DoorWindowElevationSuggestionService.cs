...
namespace BatchPdfPublisher.Services
{
    internal static class DoorWindowElevationSuggestionService
    {
        public static void Apply(DoorWindowScheduleItem item)
        {
            ...
            if (IsWindowType(type) && item.SillHeight <= 0d) item.SillHeight = 900d;
            if (!IsWindowType(type)) item.SillHeight = 0d;
        ...
        { return (type ?? string.Empty).Contains("窗") || type == "凸窗" || type == "高窗" || type == "百叶窗" || type == "门联窗"; }
